...
import { getCurrencySymbol, formatCurrency,pushNotification,sumArray } from "../functions/utility";
    ...
    async function printReciept() {
        if (purchases.length != 0) {
            ...
            await updateDoc(storeRef, {
                stocks: reducedStock,
                stock_logs:pushNotification(session,{date: new Date().getDate(),notification:`A total of ${sumArray(purchases.map(each => parseInt(each.stockUnit)))} units was purchased from the stock The stock '${purchases.map(each=> each.item).toString()}'`,tag:"debit",stock:{stockUnit:sumArray(purchases.map(each => parseInt(each.stockUnit))),type:"multiple-items"}})
            });
