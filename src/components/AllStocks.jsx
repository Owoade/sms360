...
import {formatCurrency, getCurrencySymbol, pushNotification } from "../functions/utility";
    ...
    useEffect(()=>{
      ...
      async function pushStock(e){
          ...
          await updateDoc(storeRef, {
             ...
             stock_logs:pushNotification(session,{date: new Date().getDate(),notification:`A new stock '${stockName}' was added`,tag:"credit",stock:{stockName,stockUnit:parseInt(stockUnit)},type:"single-item"})
          });
        setAddStock(null)
        setLoading(null)
      }
      async function deleteStock(stockName){
        ...
        await updateDoc(storeRef, {
            ...
            stock_logs: pushNotification(session,{date: new Date().getDate(),notification:`The stock '${stockName}' was deleted`,tag:"debit",stock:{stockName,stockUnit:parseInt(100)},type:"delete"})
         });
         setIsDeleting(false)
      }
      async function editStock(e){
        ...
        if(editState== "Increase"){
            await updateDoc(storeRef, {
                ...
                stock_logs: pushNotification(session,{date: new Date().getDate(), notification:`${stockUnit} units  was added to the stock '${addMore[0]}'`,tag:"credit",stock:{stockName:stockName,stockUnit:stockUnit},type:"single-item"})
             });
        }else{
            if((addMore[1]-stockUnit) >= 0){
                await updateDoc(storeRef, {
                    stocks:session.stocks.map(each => each.stockName == addMore[0] ? {...each,stockUnit:parseInt(addMore[1]) - parseInt(stockUnit)} : {...each} ),
                    stock_logs: pushNotification(session,{date: new Date().getDate(), notification:`${stockUnit} units was removed from the stock '${addMore[0]}'`,tag:"debit",stock:{stockName:stockName,stockUnit:stockUnit},type:"single-item"})
                 }); 
            }else{
